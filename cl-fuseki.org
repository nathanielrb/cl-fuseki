...
*** Public interface
We create three user-end functions, one to create the prefix, one to remove the prefix, and one to expand a pname.  The interface is deliberately kept as simple as possible.
...
There are some prefixes which occur all too often, we include them here by default.
